...
function setList(ret) {
    ...
    if (ret.d.Result) {
        ...
        for (var i = 0; i < order.length; i++) {
            ...
            tmprow.append($('<td>').addClass('dbody').append($('<input>').attr('type', 'button').attr('value', '所見').addClass('btn-sm').addClass('view').data('mod', order[i].Modality).data('date', order[i].Date).data('id', order[i].OrderID).data('patid', order[i].PatID).data('read', order[i].ReadDateTime)));
            ...
            tmprow.append($('<td>').append(order[i].ReadDate));
        ...
        $('input.view').on('click', function () {
            ...
            sessionStorage.setItem('read', $(this).data('read'));
            sessionStorage.setItem('cd', hospcd);
...
function postForm(url) {
    var $form = $('<form/>', { 'action': url, 'method': 'post' })
        ...
        .append($('<input/>', { type: 'hidden', name: 'read', value: sessionStorage.getItem('read') }))
